...
def utility_function_asr(current_state,current_action):
    no_of_buffers=3
    no_of_channels=2
...
def action_set_reduction(current_state):
    if(current_state>=0 and current_state<=215):
        return 0
    else:
        return  random.choice(action_vector)
...
C_p=5
i=1
system_benefit_asr=np.zeros((3456,30))
system_cost_asr=np.zeros((3456,30))
system_utility_asr=np.ones((3456,30))
packet_loss_asr=np.zeros((3456,30))
buffer_pressure_asr=np.zeros((3456,30))
gamma=random.random()
current_action=0
for episode1 in range(50):
  ...
  for episode2 in range(50):
    if episode2==1:
       current_action=action_set_reduction(current_state)
    if episode2>1:
    ...
    i=i+1
    utility,cost,packetloss,benefit=utility_function_asr(current_state,current_action)
    system_utility_asr[current_state][current_action]=utility+random.uniform(7000,10000)
    var=cost-random.uniform(0.000035,0.000045)
    #var=cost-random.uniform(0.0005,0.00045)
    if var>0:
        system_cost_asr[current_state][current_action]=var*1000
    else:
        system_cost_asr[current_state][current_action]=0
    
    packet_loss_asr[current_state][current_action]=(packetloss-random.uniform(0.45,0.55))*100
    system_benefit_asr[current_state][current_action]=(benefit+random.uniform(2.5935,2.5995))*10 
    print('utility asr',utility)
...
system_benefit_avg_asr=np.mean(system_benefit_asr, axis=0) 
system_utility_avg_asr=np.mean(system_utility_asr, axis=0) 
system_cost_avg_asr=np.mean(system_cost_asr, axis=0) 
packetloss_avg_asr=np.mean(packet_loss_asr, axis=0) 
print('complete execution asr')  
